...
class Weather(pydantic.BaseModel, BaseMergeable):
    ...
    ``precipitation``
     Precipitation type, in the form of METAR code:
      ...
      The precipitation type can be preceded by '-' for light intensity or '+' for heavy intensity. The qualifier without +/- is moderate intensity.
      For a definition and pictures of the precipitation types, see e.g. `the International Cloud Atlas <https://cloudatlas.wmo.int/en/hydrometeors-other-than-clouds-falling.html>`_
...
class Environment(pydantic.BaseModel, BaseMergeable):
    ...
    ``solar_mask``
     The solar mask at the observation site. Pandas dataframe with keys azimut (degrees from north) and elevation (degrees above horizon). See :py:class:`snowprofile.classes.SolarMask`
