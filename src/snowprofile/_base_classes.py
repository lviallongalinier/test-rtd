...
class BaseProfile(pydantic.BaseModel, BaseData):
    ...
    record_period: datetime_tuple_with_tz = pydantic.Field(
        (None, None),
        description="Time period during which the profile was done (tuple of two python datetime object representing the begin time and end time).")
    profile_depth: typing.Optional[float] = pydantic.Field(
        None, ge=0,
        description="Total snow depth at the profile location, only if different from the general total snow depth reported in the metadata (m)")
    profile_swe: typing.Optional[float] = pydantic.Field(
        None, ge=0,
        description="SWE at the profile location, only if specific measurement at the precise location of the profile (mm or kg/m2)")
...
class BaseProfile2(BaseProfile):
    ...
    uncertainty_of_measurement: typing.Optional[float] = pydantic.Field(
        None,
        gt = 0,
        description="Quantitative uncertainty of the entire profile (same units as ``data``). See :ref:`uncertainty` for details.")
